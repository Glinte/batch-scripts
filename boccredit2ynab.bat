...
REM Usage: boccredit2ynab.bat <input_file> <output_file>
REM where <input_file> is the path to the BOC xls file and <output_file> is the path to the YNAB CSV file.
REM This script simply calls the Python script boccredit2ynab.py with the provided arguments.

REM Check if the correct number of arguments is provided
if "%~1"=="" (
    echo Usage: boccredit2ynab.bat ^<input_file^> ^<output_file^>
    exit /b 1
)
set "input_file=%~1"
...
uv run "%~dp0boccredit2ynab.py" "%input_file%" "%output_file%"
